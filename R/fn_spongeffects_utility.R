...
fn_weighted_degree <- function(network, undirected = T, Alpha = 1) {
    ...
    Nodes <- data.frame(
        Nodes = union(network$geneA, network$geneB),
    ...
    Input.network <- data.frame(
        Sender = geneA.numeric,
        Receiver = geneB.numeric,
    ...
    if (undirected) {
        ...
        Nodes$Weighted_degree <- Weighted_degree$output[match(
...
define_modules <- function(
    ...
    set.parallel = F
    if (set.parallel) {
        Sponge.Modules <- foreach(
            i = 1:nrow(central.modules),
            .packages = "dplyr"
        ) %dopar%
            {
                Sponge.temp <- network %>%
                    dplyr::filter(
                        geneA == central.modules$gene[i] |
                            geneB == central.modules$gene[i]
                    )

                if (nrow(Sponge.temp) != 0) {
                    Module.temp <- union(Sponge.temp$geneA, Sponge.temp$geneB)
                    Module.temp <- Module.temp[
                        Module.temp != central.modules$gene[i]
                    ]
        ...
        for (i in 1:nrow(central.modules)) {
            ...
            if (nrow(Sponge.temp) != 0) {
                ...
                names(Sponge.Modules)[k] <- as.character(central.modules$gene)[
                    i
                ]
    ...
    if (remove.central) {
        ...
        for (z in 1:length(Sponge.Modules)) {
            ...
            Sponge.Modules.DoubleRemoved[[z]] <- Sponge.Modules[[z]][
                !(Sponge.Modules[[z]] %in% central.modules$gene)
            ]
        }
...
#'
#' @return discretized
fn_discretize_spongeffects <- function(v, n.cat) {
...
fn_OE_module <- function(
    ...
    Discretized.Genes.dist <- fn_discretize_spongeffects(
        Genes.dist,
        n.cat = bin.size
    )
    ...
    rand.scores <- fn_get_semi_random_OE(
        Centered.expr,
        Discretized.Genes.dist,
        b.sign,
...
fn_get_semi_random_OE <- function(r, genes.dist.q, b.sign, num.rounds = 1000) {
    ...
    rand.scores <- apply(B, 2, function(x) {
        sel_matrix <- r[x, ]
        if (is.array(sel_matrix)) {
            return(colMeans(sel_matrix))
        } else {
            return(mean(sel_matrix))
        }
    })
    if (is.array(rand.scores)) {
        rand.scores <- rowMeans(rand.scores)
    } else {
        rand.scores <- mean(rand.scores)
    }
...
fn_convert_gene_names <- function(
    expr_data,
    bioMart_gene_ensembl,
    bioMart_gene_symbol_columns
) {
    gene_symbols <- expr_data$Hugo_Symbol

    get_gene_map <- function(host = NULL) {
        mart <- if (is.null(host)) {
            biomaRt::useMart("ensembl", dataset = bioMart_gene_ensembl)
        } else {
            biomaRt::useMart(
                "ensembl",
                dataset = bioMart_gene_ensembl,
                host = host
            )
        }

        biomaRt::getBM(
            filters = bioMart_gene_symbol_columns,
            attributes = c(
                "ensembl_gene_id",
                bioMart_gene_symbol_columns,
                "description"
    ...
    gene_map <- tryCatch(
        ...
        error = function(first_error) {
            tryCatch(
                get_gene_map("https://useast.ensembl.org"),
                error = function(second_error) {
                    stop(
                        "Unable to query Ensembl for gene name conversion. ",
                        "Default host failed: ",
                        first_error$message,
                        "; ",
                        "useast fallback failed: ",
    ...
    matched_ensembl <- gene_map$ensembl_gene_id[
        match(gene_symbols, gene_map[[bioMart_gene_symbol_columns]])
    ]

    if (anyNA(matched_ensembl)) {
        matched_ensembl[is.na(matched_ensembl)] <- gene_symbols[is.na(
            matched_ensembl
        )]
    }

    expr_data <- expr_data[,
        setdiff(colnames(expr_data), "Hugo_Symbol"),
        drop = FALSE
    ]
    rownames(expr_data) <- matched_ensembl
...
enrichment_modules <- function(
    ...
    if (method %in% c("OE", "gsva", "ssgsea")) {
        if (method == "OE") {
            print(paste0(
                "Calculating modules with bin size: ",
                bin.size,
                ", min size: ",
                min.size,
                ", max size:",
            ...
            Enrichmentscores.modules <- foreach(
                Module = 1:length(modules),
                .packages = c("dplyr", "GSVA"),
                .export = c(
                    "fn_OE_module",
            ...
            ) %dopar%
                {
                    ...
                    if (
                        length(modules[[Module]]) > min.size &
                            length(modules[[Module]]) < max.size
                    ) {
                        if (
                            sum(
                                (modules[[Module]] %in% rownames(Expr.matrix)),
                                na.rm = TRUE
                            ) >
                                min.expr
                        ) {
                            Enrichment.module <- round(
                                fn_OE_module(
                                    Expr.matrix,
                                    modules[[Module]],
                                    bin.size
                            ...
                            colnames(Enrichment.module) <- names(modules)[
                                Module
                            ]
            ...
            Enrichmentscores.modules <- Enrichmentscores.modules %>%
                t() %>%
                as.data.frame

            colnames(Enrichmentscores.modules) <- colnames(Expr.matrix)
        } else {
            Enrichmentscores.modules <- GSVA::gsva(
                as.matrix(Expr.matrix),
                modules,
                min.sz = min.size,
                max.sz = max.size,
                method = method,
...
fn_exact_match_summary <- function(data, lev = NULL, model = NULL) {
...
fn_RF_classifier <- function(
    ...
    Output.rf <- list(
        Model = rf.model,
        ConfusionMatrix_training = Confusion.matrix
    )
    return(Output.rf)
}
...
prepare_tcga_for_spongEffects <- function(
    tcga_cancer_symbol,
    ...
    TCGA.meta.tumor$PATIENT_ID <- substr(
        TCGA.meta.tumor$sampleID,
        1,
        nchar(TCGA.meta.tumor$sampleID) - 3
    )
    ...
    MetaData_TCGA_Complete <- clinical_data %>%
        dplyr::filter(
            !(.data$SUBTYPE %in% c("")) &
                .data$PATIENT_ID %in% TCGA.meta.tumor$PATIENT_ID
        ) %>%
        dplyr::filter(
            AJCC_PATHOLOGIC_TUMOR_STAGE %in% tumor_stages_of_interest
        ) %>%
        mutate(
            Grading = ifelse(
                ...
                ifelse(
                    ...
                    "Stage_II",
                    ifelse(
                        AJCC_PATHOLOGIC_TUMOR_STAGE %in% "STAGE IV",
                        "Stage_IV",
                        "Stage_III"
                    )
                )
    ...
    TCGA.meta.tumor <- TCGA.meta.tumor[
        match(MetaData_TCGA_Complete$PATIENT_ID, TCGA.meta.tumor$PATIENT_ID),
    ] %>%
        left_join(MetaData_TCGA_Complete, .by = PATIENT_ID)
    TCGA.meta.tumor$SUBTYPE <- gsub(
        tcga_cancer_symbol,
        "",
        TCGA.meta.tumor$SUBTYPE
    )
    TCGA.meta.tumor$SUBTYPE <- factor(
        TCGA.meta.tumor$SUBTYPE,
        levels = subtypes_of_interest
    )

    TCGA.expr.normal <- TCGA.expr.normal[, match(
        TCGA.meta.normal$sampleID,
        colnames(TCGA.expr.normal)
    )]
    TCGA.expr.tumor <- TCGA.expr.tumor[, match(
        TCGA.meta.tumor$sampleID,
        colnames(TCGA.expr.tumor)
    )]

    rownames(TCGA.expr.tumor) <- gsub("\\..*", "", rownames(TCGA.expr.tumor))
    rownames(TCGA.expr.normal) <- gsub("\\..*", "", rownames(TCGA.expr.normal))

    prep_TCGA_spongEffects <- list(
        TCGA.expr.tumor,
        TCGA.expr.normal,
        TCGA.meta.tumor,
        TCGA.meta.normal,
        MetaData_TCGA_Complete
    )
    names(prep_TCGA_spongEffects) <- c(
        "TCGA.expr.tumor",
        "TCGA.expr.normal",
        "TCGA.meta.tumor",
        "TCGA.meta.normal",
...
prepare_metabric_for_spongEffects <- function(
    metabric_expression,
    metabric_metadata,
    subtypes_of_interest,
    ...
    METABRIC.expr <- read.delim(metabric_expression, header = T) %>%
        dplyr::select(-Entrez_Gene_Id)
    METABRIC.expr <- METABRIC.expr[
        Biobase::isUnique(METABRIC.expr$Hugo_Symbol),
    ]

    METABRIC.expr <- fn_convert_gene_names(
        METABRIC.expr,
        bioMart_gene_ensembl,
        bioMart_gene_symbol_columns
    )
    ...
    METABRIC.meta <- read.delim(metabric_metadata, comment.char = "#") %>%
        filter(
            CLAUDIN_SUBTYPE %in%
                subtypes_of_interest &
                PATIENT_ID %in% colnames(METABRIC.expr)
        )
    METABRIC.meta$CLAUDIN_SUBTYPE <- factor(
        METABRIC.meta$CLAUDIN_SUBTYPE,
        levels = subtypes_of_interest
    )

    METABRIC.expr <- METABRIC.expr[, match(
        METABRIC.meta$PATIENT_ID,
        colnames(METABRIC.expr)
    )]
...
filter_ceRNA_network <- function(
    ...
    Node_Centrality <- Node_Centrality %>%
        dplyr::filter(
            gene %in% Sponge.filtered$geneA | gene %in% Sponge.filtered$geneB
        )
    ...
    if (!add_weighted_centrality) {
        ...
        Node_Centrality <- Node_Centrality %>%
            mutate(
                Weighted_Degree = Nodes$Weighted_degree[match(
                    Node_Centrality$gene,
                    Nodes$Nodes
                )]
            )
        sponge_network_centralites <- list(Sponge.filtered, Node_Centrality)
        names(sponge_network_centralites) <- c(
            "Sponge.filtered",
            "Node_Centrality"
        )
    }
...
get_central_modules <- function(
    ...
    if (centrality_measure %in% colnames(node_centrality)) {
        if ("circRNA" %in% ceRNA_class) {
            df_circRNAS_to_add <- node_centrality[with(
                node_centrality,
                grepl("circ", gene) | grepl(":", gene) | grepl("chr", gene)
            )]
        ...
        print(paste0(
            "centrality_measure must be one of the following: ",
            colnames(node_centrality)
        ))
    }
}
...
calibrate_model <- function(
    ...
    if (
        label %in%
            colnames(modules_metadata) &
            sampleIDs %in% colnames(modules_metadata)
    ) {
        # Train subtype classifier -------------------------------------------------------------
        # Define input
        Inputdata.model <- t(Input) %>%
            scale(center = T, scale = T) %>%
            as.data.frame()
        Inputdata.model <- Inputdata.model %>%
            mutate(
                Class = as.factor(modules_metadata[
                    match(
                        rownames(Inputdata.model),
                        modules_metadata[, sampleIDs]
                    ),
                    label
                ])
            )
        ...
        # Calibrate model
        SpongingActivity.model <- fn_RF_classifier(
            Input.object = Inputdata.model,
            K = n.folds,
            rep = repetitions,
            metric = Metric,
            tunegrid = tunegrid
...
build_classifier_central_genes <- function(
    train_gene_expr,
    test_gene_expr,
    ...
    Common.CentralGenes <- intersect(
        rownames(METABRIC.expr),
        intersect(rownames(TCGA.expr.tumor), rownames(BRCA.Modules.OE))
    )

    # Define model input
    #TCGA.expr.tumor <- TCGA.expr.tumor[, match(TCGA.meta.tumor$sampleID, colnames(TCGA.expr.tumor))]

    Inputdata.centralGene <- t(TCGA.expr.tumor[
        rownames(TCGA.expr.tumor) %in% Common.CentralGenes,
    ]) %>%
    ...
    if (train_meta_data_type == "TCGA") {
        Inputdata.centralGene <- Inputdata.centralGene %>%
            mutate(
                Class = as.factor(TCGA.meta.tumor$SUBTYPE[match(
                    rownames(Inputdata.centralGene),
                    TCGA.meta.tumor$sampleID
                )])
            )
    }
    if (train_meta_data_type == "METABRIC") {
        Inputdata.centralGene <- Inputdata.centralGene %>%
            mutate(
                Class = as.factor(TCGA.meta.tumor$CLAUDIN_SUBTYPE[match(
                    rownames(Inputdata.centralGene),
                    TCGA.meta.tumor$PATIENT_ID
                )])
            )
    }

    Inputdata.centralGene <- Inputdata.centralGene[
        !is.na(Inputdata.centralGene$Class),
    ]

    CentralGenes.model <- fn_RF_classifier(
        Input.object = Inputdata.centralGene,
        K = n.folds,
        rep = repetitions,
        metric = Metric,
        tunegrid = tunegrid
    ...
    #Test classification performance on second cohort
    Inputdata.centralGene.Metabric <- t(METABRIC.expr[
        Common.CentralGenes,
    ]) %>%
        scale(center = TRUE, scale = TRUE)
    Prediction.centralGenes.model <- predict(
        CentralGenes.model$Model,
        Inputdata.centralGene.Metabric
    )

    if (test_meta_data_type == "TCGA") {
        CentralGenes.model$ConfusionMatrix_testing <- confusionMatrix(
            as.factor(Prediction.centralGenes.model),
            as.factor(METABRIC.meta$SUBTYPE)
        )
    }
    if (test_meta_data_type == "METABRIC") {
        CentralGenes.model$ConfusionMatrix_testing <- confusionMatrix(
            as.factor(Prediction.centralGenes.model),
            as.factor(METABRIC.meta$CLAUDIN_SUBTYPE)
        )
    }
...
#' @param min.size minimum module size (default: 10)
#' @param max.size maximum module size (default: 200)
#' @param min.expression minimum expression (default: 10)
#' @param method Enrichment to be used (Overall Enrichment: OE or Gene Set
#' Variation Analysis: GSVA) (default: OE)
...
Random_spongEffects <- function(
    ...
    Random.modules <- enrichment_modules(
        gene_expr,
        Random.Modules,
        bin.size = bin.size,
        min.size = min.size,
        max.size = max.size,
        method = method,
        min.expr = min.expression,
        cores
    )

    return(list(
        Random_Modules = Random.Modules,
        Enrichment_Random_Modules = Random.modules
    ))
}
...
plot_top_modules <- function(
    ...
    Variable.importance <- importance(final.model) %>%
        as.data.frame() %>%
    ...
    p <- Variable.importance[1:k_modules, ] %>%
        mutate(
            Analysed = c(rep("1", k_modules_red), rep("0", grey_modules))
        ) %>%
        arrange(desc(MeanDecreaseGini)) %>%
        ggplot(aes(
            x = reorder(Module, MeanDecreaseGini),
            y = MeanDecreaseGini
        )) +
        geom_point() +
        geom_segment(aes(
...
plot_density_scores <- function(
    ...
    if (label %in% colnames(meta_data) & sampleIDs %in% colnames(meta_data)) {
        ...
        Density.plot <- spongEffects[
            rownames(spongEffects) %in% Variable.importance$Module,
        ] %>%
            gather(Patient, Score) %>%
            mutate(
                Class = meta_data[match(Patient, meta_data[, sampleIDs]), label]
            ) %>%
...
plot_accuracy_sensitivity_specificity <- function(
    trained_model,
    ...
    if (central_genes && !all_expression) {
        Accuracy.df <- data.frame(
            ...
            Accuracy = c(
                SpongingActiivty.model$ConfusionMatrix_training[["overall"]][[
                    'Accuracy'
                ]],
                SpongingActiivty.model$ConfusionMatrix_testing[["overall"]][[
                    'Accuracy'
                ]],
                CentralGenes.model$ConfusionMatrix_training[["overall"]][[
                    'Accuracy'
                ]],
                CentralGenes.model$ConfusionMatrix_testing[["overall"]][[
                    'Accuracy'
                ]],
                Random.model$ConfusionMatrix_training[["overall"]][[
                    'Accuracy'
                ]],
        ...
        Accuracy.df$Model <- factor(
            Accuracy.df$Model,
            levels = c("Modules", "Random", "Central Genes")
        )
        ...
        Accuracy.df <- data.frame(
            ...
            Accuracy = c(
                ...
                Random.model$ConfusionMatrix_testing[["overall"]][['Accuracy']],
                all_expression_model$ConfusionMatrix_training[["overall"]][[
                    'Accuracy'
                ]],
                all_expression_model$ConfusionMatrix_testing[["overall"]][[
                    'Accuracy'
                ]]
            )
        )
        ...
        Accuracy.df$Model <- factor(
            Accuracy.df$Model,
            levels = c("Modules", "Random", "Central Genes", "All Expression")
        )
        Accuracy.df$Run <- factor(Accuracy.df$Run, levels = set_names)
    }
    ...
    # Figure 3b
    Metrics.SpongeModules.training <- SpongingActiivty.model$ConfusionMatrix_training[[
        "byClass"
    ]][c(1:length(unique(subtypes)))] %>%
        as.data.frame() %>%
        mutate(Model = "Modules") %>%
        tibble::rownames_to_column('Class') #%>% gather(Metric, Value, Sensitivity:Specificity,-Class, -Model)

    colnames(Metrics.SpongeModules.training) = c("Class", "Value", "Model")

    Metrics.Random.training <- Random.model$ConfusionMatrix_training[[
        "byClass"
    ]][c(1:length(unique(subtypes)))] %>%
        as.data.frame() %>%
        mutate(Model = "Random") %>%
    ...
    if (central_genes && !all_expression) {
        Metrics.CentralGenes.training <- CentralGenes.model$ConfusionMatrix_training[[
            "byClass"
        ]][c(1:length(unique(subtypes)))] %>%
            as.data.frame() %>%
            mutate(Model = "Central Genes") %>%
            tibble::rownames_to_column('Class') #%>% gather(Metric, Value, Sensitivity:Specificity,-Class, -Model)
        colnames(Metrics.CentralGenes.training) = c("Class", "Value", "Model")

        Metrics.training <- rbind(
            Metrics.SpongeModules.training,
        ...
        Metrics.AllExpression.training <- all_expression_model$ConfusionMatrix_training[[
            "byClass"
        ]][c(1:length(unique(subtypes)))] %>%
            as.data.frame() %>%
            mutate(Model = "All Expression") %>%
            tibble::rownames_to_column('Class') #%>% gather(Metric, Value, Sensitivity:Specificity,-Class, -Model)
        ...
        Metrics.training <- rbind(
            ...
            rbind(
                Metrics.Random.training,
                rbind(
                    Metrics.CentralGenes.training,
                    Metrics.AllExpression.training
                )
            )
        ) %>%
            mutate(Run = training_string)
    ...
    Metrics.SpongeModules.testing <- SpongingActiivty.model$ConfusionMatrix_testing[[
        "byClass"
    ]][c(1:length(unique(subtypes)))] %>%
        as.data.frame() %>%
        mutate(Model = "Modules") %>%
    ...
    Metrics.Random.testing <- Random.model$ConfusionMatrix_testing[[
        "byClass"
    ]][c(1:length(unique(subtypes)))] %>%
        as.data.frame() %>%
        mutate(Model = "Random") %>%
    ...
    if (central_genes && !all_expression) {
        Metrics.CentralGenes.testing <- CentralGenes.model$ConfusionMatrix_testing[[
            "byClass"
        ]][c(1:length(unique(subtypes)))] %>%
            as.data.frame() %>%
            mutate(Model = "Central Genes") %>%
        ...
        Metrics.testing <- rbind(
            Metrics.SpongeModules.testing,
        ...
        Metrics.AllExpression.testing <- all_expression_model$ConfusionMatrix_testing[[
            "byClass"
        ]][c(1:length(unique(subtypes)))] %>%
            as.data.frame() %>%
            mutate(Model = "All Expression") %>%
        ...
        Metrics.testing <- rbind(
            ...
            rbind(
                Metrics.Random.testing,
                rbind(
                    Metrics.CentralGenes.testing,
                    Metrics.AllExpression.testing
                )
            )
        ) %>%
            mutate(Run = testing_string)
...
#'
#' @param trained_model returned from train_and_test_model
#' @param subtypes.testing.factors subtypes of testing samples as factors
#' @return plot of the confusion matrix
#'
#' @export
#'
#' @return returns confusion matrix plots of the trained model
plot_confusion_matrices <- function(trained_model, subtypes.testing.factors) {
    trained.model <- trained_model
    ...
    Prediction.SpongeModules <- data.frame(
        Predicted = as.factor(trained.model$Prediction.model),
        Observed = as.factor(subtypes.testing.factors)
    )
    Prediction.SpongeModules.cm <- confusion_matrix(
        targets = Prediction.SpongeModules$Observed,
        predictions = Prediction.SpongeModules$Predicted
    )

    Confusion.matrix.SpongeModules <- cvms::plot_confusion_matrix(
        Prediction.SpongeModules.cm$`Confusion Matrix`[[1]],
        add_sums = T,
...
plot_heatmaps <- function(
    ...
    if (label %in% colnames(meta_data) & sampleIDs %in% colnames(meta_data)) {
        ...
        Column.Annotation <- HeatmapAnnotation(
            Group = Annotation.meta[, label],
            col = list(Group = col.heatmap)
        )

        spongeEffects.toPlot <- spongEffects[
            match(Variable.importance$Module, rownames(spongEffects)),
        ]
        spongeEffects.toPlot <- spongeEffects.toPlot[
            rowSums(is.na(spongeEffects.toPlot)) == 0,
        ]
...
#'
#' @return plot object
plot_involved_miRNAs_to_modules <- function(
    sponge_modules,
    trained_model,
    ...
    ensembl_data_env <- new.env(parent = emptyenv())
    utils::data("ensembl.df", package = "SPONGE", envir = ensembl_data_env)
    ensembl.df_local <- ensembl_data_env$ensembl.df

    get_local_gene_label <- function(ensg_id) {
        matched_index <- match(ensg_id, ensembl.df_local$ensembl_gene_id)
        if (is.na(matched_index)) {
            return(ensg_id)
        }

        gene_label <- ensembl.df_local$description[matched_index]
        gene_label <- sub(" \\[Source:.*$", "", gene_label)

        if (is.na(gene_label) || length(gene_label) == 0 || gene_label == "") {
            gene_label <- ensg_id
        }
    ...
    for (ensg in names(Sponge.modules.downstrean)) {
        ...
        df_intern = data.frame(
            ensembl_gene_id = ensg,
            symbol = get_local_gene_label(ensg),
            stringsAsFactors = FALSE
        )

        vec_colnames[count] = df_intern$symbol
    ...
    for (ensg in vec_colnames_ensg) {
        ...
        df_symbolname = df_centralnodes_map[
            which(df_centralnodes_map$ensembl_gene_id == ensg),
        ]

        module_symbole = df_symbolname$symbol

        if (
            length(rownames(df_symbolname)) == 0 ||
                is.na(module_symbole) ||
                module_symbole == ""
        ) {
            module_symbole = ensg
        }
        ...
        df_mirna_counts = data.frame(matrix(
            ncol = length(df_mirnas_intern$mirna)
        ))
        ...
        for (ensg_target in df_intern$Geneid) {
            #ensg_target="ENSG00000064042"

            df_symbolname = df_intern[
                which(df_intern$Geneid == ensg_target),
            ]
            ...
            if (!is.na(symbolname)) {
                miRNAs_thistarget <- miRNAs_significance.downstream.Target[[
                    symbolname
                ]]$mirna
                # miRNAs_thistarget=miRNA_AccessionToName(miRNAs_thistarget)

                intersect_mirnas = intersect(
                    miRNAs_thistarget,
                    df_mirnas_intern$mirna
                )
        ...
        for (mirna_name in colnames(df_mirna_counts)) {
            ...
            if (!is.na(mirna_name)) {
                if (!any(row.names(df_heatmap) == mirna_name)) {
                    ...
                    rownames(df_heatmap)[length(rownames(
                        df_heatmap
                    ))] = mirna_name
                }
    ...
    if (show_annotation_column) {
        ...
        heatmap.miRNA <- df_heatmap %>%
            Heatmap(
                na_col = "white",
                col = col.heatmap, # right_annotation = row.Annotation,
                ...
                heatmap_legend_param = list(
                    ...
                    legend_width = unit(legend_width, unit),
                ...
                row_names_gp = gpar(fontsize = row_names_gp_fontsize),
                column_names_gp = gpar(fontsize = column_names_gp_fontsize),
                column_names_rot = column_names_rot,
                rect_gp = gpar(col = "white", lwd = 0.5),
                width = unit(width, unit),
                height = unit(length, unit)
            )
    } else {
        # Heatmap
        heatmap.miRNA <- df_heatmap %>%
