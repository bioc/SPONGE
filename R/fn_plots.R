...
sponge_network <- function(
    ...
    genes <- unique(c(
        as.character(sponge_result$geneA),
        as.character(sponge_result$geneB)
    ))
    ...
    if (show.sponge.interaction) {
        edges <- rbind(edges, sponge.edges)
    }

    #sponge gene nodes
    sponge.nodes <- data.frame(
        id = genes,
        label = genes,
        shape = "square",
        color = "darkgreen",
    ...
    if (show.mirnas != "none") {
        mirna.edges <- foreach(gene = nodes$id, .combine = rbind) %do%
            {
                ...
                foreach(
                    mir = iterators::iter(gene_mirnas, by = "row"),
                    .combine = rbind
                ) %do%
                    {
                        with(mir, {
        ...
        if (show.mirnas == "shared") {
            ...
            mirnas <- intersect(
                mirnas,
                names(which(
                    table(as.character(mirna.edges$to)) > min.interactions
                ))
            )

            mirna.edges <- mirna.edges[which(mirna.edges$to %in% mirnas), ]
        } else {
            mirnas <- unique(as.character(mirna.edges$to))
        }
    ...
    nodes <- nodes[
        which(
            nodes$id %in% c(as.character(edges$to), as.character(edges$from))
        ),
    ]
...
sponge_plot_network <- function(
    ...
    if (nrow(edges) < 10000) {
        ...
        plot <- visNetwork::visIgraphLayout(
            plot,
            layout = layout,
            type = "full",
            physics = force.directed
        )
        plot <- visNetwork::visOptions(
            plot,
        ...
        stop(
            "With more than 10000 edges, this plot is omitted for performance reasons"
        )
    }
}
