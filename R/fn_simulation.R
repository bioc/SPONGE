...
schur <- function(R) {
    if (ncol(R) < 3) {
        stop("Input matrix needs to have dimensions 3x3 or larger")
    }
    if (nrow(R) != ncol(R)) {
        stop("Input matrix needs to be square")
    }
    ((R[1:2, 1:2]) -
        (R[1:2, 3:ncol(R)]) %*%
            (solve(R[3:nrow(R), 3:ncol(R)])) %*%
            t(R[1:2, 3:ncol(R)]))
}
...
get.q <- function(S) {
    ...
    q <- s11 /
        outer(sqrt(diag(s11)), sqrt(diag(s11))) -
        pva /
...
checkLambda <- function(x, y) {
    ...
    beta <- foreach(i = seq_along(x), .combine = sum) %do%
        {
            x[i] * y[i]
        }
...
sample_zero_mscor_cov <- function(
    m,
    number_of_solutions,
    ...
    solutions <- foreach(
        solution = seq_len(number_of_solutions),
        .packages = c("MASS", "gRbase", "ppcor", "logger", "foreach", "expm"),
        .export = c(
            "quadraticSolver",
            "checkLambda",
            "get.q",
    ...
    ) %dorng%
        {
            ...
            log_debug(
                paste(
                    "Looking for zero sensitivity covariance matrix for case m =",
                    m,
                    "solution no.",
                    solution
                )
            ...
            while (total < number_of_attempts) {
                total <- total + 1
                if (is.null(gene_gene_correlation)) {
                    # R11
                    K = runif(1, -1, 1)
                } else {
                    K = gene_gene_correlation
                }
                ...
                if (m == 1) {
                    ...
                    r12.m <- (K -
                        sqrt(u1.norm^2) * sqrt(u2.norm.1^2) * lambda) *
                        (1 - u1.norm^2)^-(1 / 2) *
                        (1 - u2.norm.1^2)^-(1 / 2)

                    if (is.nan(r12.m)) {
                        r12.m <- (K -
                            sqrt(u1.norm^2) * sqrt(u2.norm.2^2) * lambda) *
                            (1 - u1.norm^2)^-(1 / 2) *
                            (1 - u2.norm.2^2)^-(1 / 2)
                    }
                    ...
                    if (r12.m != K) {
                        log_debug(
                            "correlation is not equal to partial correlation for selected ||u2||"
                        )
                        next
                    }
                    ...
                    if (anyNA(constraints)) {
                        log_debug(
                            "R22^-(1/2) invalid. Can not compute constraints"
                        )
                        next
                    }
...
sample_zero_mscor_data <- function(
    cov_matrices,
    ...
    foreach(cov.matrix = cov_matrices) %do%
        {
            ...
            if (
                abs(cov2pcor(cov.matrix)[1, 2] - cov2cor(cov.matrix)[1, 2]) >
                    sqrt(.Machine$double.eps)
            ) {
                stop(
                    "sensitivity correlation of a given covariance matrix is not zero."
                )
            }
